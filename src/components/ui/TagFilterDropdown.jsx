              ...
              suggestions.map(({ tag, count, isNew }, index) => (
                ...
                >
                  <div className="flex items-center gap-2 min-w-0 flex-1">
                    {isNew ? (
                      <Plus className={`h-4 w-4 shrink-0 ${index === activeIndex ? 'text-[#D48459]' : 'text-primary'}`} />
                    ) : (
                      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640" className={`h-4 w-4 shrink-0 ${index === activeIndex ? 'text-[#D48459]' : 'text-muted-foreground'}`} fill="currentColor" aria-hidden="true" style={index === activeIndex ? { color: '#D48459' } : {}}>
                    ...
                    <span className="truncate text-left">{normalizeTagName(tag)}</span>
                  </div>
                  {!isNew && count > 0 && (
