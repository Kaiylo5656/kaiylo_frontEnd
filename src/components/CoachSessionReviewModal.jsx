...
const CoachSessionReviewModal = ({ isOpen, onClose, session, selectedDate, studentId }) => {
  ...
  const [showExercises, setShowExercises] = useState(window.innerWidth >= 768);

  useEffect(() => {
    const handleResize = () => {
      const mobile = window.innerWidth < 768;
      setIsMobile(mobile);
      if (!mobile) setShowExercises(true);
    };
  ...
  const updateExercisesPosition = useCallback(() => {
    const mobile = window.innerWidth < 768;
    const panelWidth = mobile ? window.innerWidth - 32 : 340;
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      window.addEventListener('resize', updateExercisesPosition);
      return () => window.removeEventListener('resize', updateExercisesPosition);
    }
        ...
        >
          ...
          {isOpen && showExercises && (
            <SessionExercisesModal
              isOpen={isOpen}
              onClose={() => isMobile && setShowExercises(false)}
              session={session}
        ...
        >
          ...
          <div className="flex-1 min-h-0 overflow-y-auto overscroll-contain modal-scrollable-body px-6 pt-3 pb-6 space-y-5">
            ...
            <div className={`flex gap-[20px] mb-[20px] ${isMobile ? 'flex-col' : ''}`}>
              ...
              {/* Right: Video Player */}
              <div className={`flex-1 flex flex-col ${isMobile ? 'min-w-0 w-full' : 'min-w-[300px]'}`}>
                {currentSetVideo && currentSetVideo.video_url ? (
                  <div className="bg-[#1A1A1D] rounded-[10px] shadow-[0px_5px_10px_0px_rgba(0,0,0,0.25)] relative min-h-[320px] flex flex-col pt-[14px] px-[14px] pb-[14px]">
                    ...
                    <div className="bg-black rounded-[10px] shadow-[0px_4px_10px_0px_rgba(0,0,0,0.5)] overflow-hidden mb-[12px] flex-shrink-0 relative flex flex-col">
                      {/* Video Player */}
                      <div className={`${isMobile ? 'aspect-video w-full h-auto' : 'h-[158px] w-full'} overflow-hidden`}>
                        <video
            ...
            <div className={`flex gap-3 pt-0 ${isMobile ? 'flex-col-reverse' : 'justify-end'}`}>
              <button
                type="button"
                onClick={onClose}
                className={`px-5 py-2.5 text-sm font-extralight text-white/70 bg-[rgba(0,0,0,0.5)] rounded-[10px] hover:bg-[rgba(255,255,255,0.1)] transition-colors ${isMobile ? 'w-full' : ''}`}
              >
                Annuler
              </button>
              <button
                ...
                disabled={savingFeedback}
                className={`px-5 py-2.5 text-sm font-normal bg-primary text-primary-foreground rounded-[10px] hover:bg-primary/90 transition-colors disabled:opacity-50 ${isMobile ? 'w-full' : ''}`}
