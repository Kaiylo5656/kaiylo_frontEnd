...
const DeleteExerciseModal = ({ 
  ...
  const modalId = 'delete-exercise-modal';
  ...
  const isMultiple = exerciseCount > 1;
  const exerciseText = exerciseCount === 1 ? 'exercice' : 'exercices';
  ...
  return (
    <BaseModal
      ...
      size="md"
      title={isMultiple ? `Supprimer ${exerciseCount} ${exerciseText}` : 'Supprimer l\'exercice'}
        ...
        <div className="flex flex-col items-start space-y-4">
          <div className="text-left space-y-2">
            <p className="text-sm font-extralight text-white/70">
              {isMultiple ? (
                <>
                  Êtes-vous sûr de vouloir supprimer <span className="font-normal text-white">{exerciseCount} {exerciseText}</span> ?
                </>
              ) : (
                <>
                  Êtes-vous sûr de vouloir supprimer l'exercice <span className="font-normal text-white">"{exerciseTitle}"</span> ?
                </>
              )}
            </p>
          </div>
