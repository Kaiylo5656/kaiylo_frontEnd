  ...
  if (embedded) {
    return (
      <div className="flex-1 min-h-0 px-6 py-6 overflow-y-auto overscroll-contain modal-scrollable-body" style={{ overflowX: 'hidden' }}>
        <Reorder.Group axis="y" values={exercises} onReorder={onReorder || (() => { })} className="space-y-1.5" style={{ padding: 0, margin: 0, listStyleType: 'none' }}>
          {exercises.map((exercise, index) => (
            <Reorder.Item
              ...
              onPointerEnter={() => setHoveredId(exercise.id)}
              onPointerLeave={() => setHoveredId(null)}
              animate={{
                backgroundColor: draggingId === exercise.id || (hoveredId === exercise.id && !draggingId)
                  ? 'rgba(212, 132, 90, 0.25)'
                  : 'rgba(0, 0, 0, 0.5)',
                scale: draggingId === exercise.id ? 1.02 : 1,
                boxShadow: draggingId === exercise.id ? '0 8px 20px rgba(0,0,0,0.3)' : 'none',
                zIndex: draggingId === exercise.id ? 50 : 1
              }}
