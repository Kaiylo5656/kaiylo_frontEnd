...
const OneRmModal = ({
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      // Ouvrir l'historique par défaut quand la modal s'ouvre (seulement sur desktop)
      setIsHistoryModalOpen(window.innerWidth >= 768);
    } else {
  ...
  return (
    ...
    >
      <div ref={containerRef} className="relative">
        <div
          ref={contentRef}
          className={`relative mx-auto max-h-[92vh] overflow-visible rounded-2xl shadow-2xl flex flex-col w-[95vw] md:w-auto md:min-w-[387px] ${isMobile && isHistoryModalOpen ? 'h-[500px]' : ''}`}
          style={{
        ...
        >
          ...
          <div className="shrink-0 px-6 pt-6 pb-3 flex items-center justify-between gap-4">
            <div className="flex items-center gap-3 min-w-0 flex-1">
              {isMobile && isHistoryModalOpen ? (
                <div className="flex items-center gap-3 w-full">
                  <button
                    onClick={() => setIsHistoryModalOpen(false)}
                    className="p-1 -ml-2 transition-colors hover:opacity-90"
                  ...
                  <h2 className="text-xl font-normal text-white flex items-center gap-2" style={{ color: 'var(--kaiylo-primary-hex)' }}>
                    Historique - <span className="font-light">{selectedLift?.name}</span>
                  </h2>
                </div>
              ) : (
                <>
            ...
            <div className="flex items-center gap-3 flex-shrink-0">
              ...
              {!isHistoryModalOpen && isMobile && (
                <button
                  onClick={() => setIsHistoryModalOpen(true)}
                  className="transition-colors p-1 hover:opacity-90"
                  style={{ color: 'var(--kaiylo-primary-hex)' }}
                  aria-label="Voir l'historique"
                >
                  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 576 512" className="h-5 w-5" fill="currentColor" aria-hidden="true">
                ...
                <div className="rounded-none divide-y divide-white/5 flex flex-col gap-[6px]">
                  {editedLifts.map((lift) => {
                    ...
                    return (
                      <div
                        key={lift.id}
                        className={`w-full flex items-center gap-4 px-4 py-2.5 transition-all duration-150 rounded-2xl border-t-0 ${isActive
        ...
        {!isMobile && (
          ...
          >
            <button
              ...
              className={`inline-flex h-10 w-10 items-center justify-center rounded-full transition-all duration-150 ${isHistoryModalOpen
