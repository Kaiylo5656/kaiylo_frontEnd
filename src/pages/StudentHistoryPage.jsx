...
const StudentHistoryPage = () => {
  ...
  // Séances passées du mois : terminées, non réalisées (skipped, failed), ou assignées dont la date est passée
  const pastSessionsThisMonth = useMemo(() => {
    const monthStart = startOfMonth(currentMonth);
    const monthEnd = endOfMonth(currentMonth);
    const today = startOfDay(new Date());
    return assignments
      .filter((a) => {
        ...
        if (!dateStr) return false;
        const d = parseISO(dateStr);
        const dateInMonth = d >= monthStart && d <= monthEnd;
        const isPast = d < today;
        const isFinalized = ['completed', 'skipped', 'failed'].includes((a.status || '').toLowerCase());
        return dateInMonth && (isPast || isFinalized);
      })
      .map((a) => ({
        id: a.id,
        assignmentId: a.id,
  ...
  // Séances passées à afficher : tout le mois ou uniquement le jour sélectionné
  const pastSessionsToDisplay = useMemo(() => {
    if (showAllPastSessions) return pastSessionsThisMonth;
    const selectedDateStr = format(selectedDate, 'yyyy-MM-dd');
    return pastSessionsThisMonth.filter(
      (s) => format(parseISO(s.scheduled_date), 'yyyy-MM-dd') === selectedDateStr
    );
  }, [pastSessionsThisMonth, selectedDate, showAllPastSessions]);

  const getPastSessionStatusLabel = (status) => {
    const s = (status || '').toLowerCase();
    if (s === 'completed') return null;
    if (s === 'skipped') return 'Reportée';
  ...
  const handleDayClick = (day) => {
    if (day) {
      setSelectedDate(day);
      setSessionListView('past');
      setShowAllPastSessions(false); // Afficher uniquement les séances du jour cliqué
    }
  };
  ...
  return (
    ...
    >
                ...
                return (
                  <button
                    ...
                    className={`flex-1 flex flex-col gap-[15px] items-center px-0 py-[5px] min-w-0 h-[50px] transition-colors ${isSelected
                      ? 'bg-white/5 border-[0.5px] border-[rgba(255,255,255,0.05)] rounded-[7.5px]'
                      : 'hover:bg-white/5'
                      }`}
                  >
                    <div className={`flex flex-col gap-[5px] items-center ${isToday ? 'bg-[#d4845a] rounded-full px-[6px] py-[2px] shrink-0 flex items-center justify-center' : ''
                      }`}>
                      <p className={`text-[10px] font-normal leading-normal text-center ${(isToday || isSelected) ? 'text-white' : 'text-white/50'
                        }`}>
                        {format(day, 'd')}
                      </p>
                    </div>
      ...
      {(upcomingSessions.length > 0 || pastSessionsThisMonth.length > 0) && (
        <div className="px-4 pb-16 pt-6 relative z-10">
          <div className="flex bg-[rgba(255,255,255,0.05)] border border-white/10 rounded-full p-1 mb-3">
            <button
              ...
              className={`flex-1 px-1.5 py-1.5 text-sm rounded-full transition-all duration-200 ${sessionListView === 'upcoming'
            ...
            <button
              ...
              className={`flex-1 px-1.5 py-1.5 text-sm rounded-full transition-all duration-200 ${sessionListView === 'past'
                ? 'bg-[#e87c3e] text-white shadow-lg font-normal'
                : 'text-white/50 hover:text-white font-light'
                }`}
            >
              Séances passées ({pastSessionsThisMonth.length})
            </button>
          </div>
          ...
          {sessionListView === 'past' && (
            <div className="flex flex-col gap-2">
              {!showAllPastSessions && (
                <button
                  type="button"
                  onClick={() => setShowAllPastSessions(true)}
                  className="w-full py-2 px-3 text-sm text-white/60 hover:text-white font-light rounded-full bg-white/5 border border-white/10 hover:border-white/20 transition-colors mb-2"
                >
                  Afficher toutes les séances passées ({pastSessionsThisMonth.length})
                </button>
              )}
              {pastSessionsToDisplay.length === 0 ? (
                <p className="text-sm text-white/40 py-4 text-center">
                  {pastSessionsThisMonth.length === 0
                    ? 'Aucune séance passée ce mois'
                    : showAllPastSessions
                      ? 'Aucune séance passée ce mois'
                      : `Aucune séance passée le ${format(selectedDate, 'd MMMM', { locale: fr })}`}
                </p>
              ) : (
                <>
                  {pastSessionsToDisplay.map((session, idx) => {
                    const sessionId = session.id || session.assignmentId || idx;
                    ...
                    const scheduledDateFormatted = session.scheduled_date
                      ? format(parseISO(session.scheduled_date), 'd MMM yyyy', { locale: fr })
                      : '';
                    const statusLabel = getPastSessionStatusLabel(session.status);
                    const toggleExpanded = () => {
                        ...
                        >
                          <div className="flex items-center gap-1 min-w-0 flex-1">
                            ...
                            {statusLabel && (
                              <>
                                <span className="text-[13px] text-white/50 flex-shrink-0"> · </span>
                                <span className="text-[11px] text-white/50 flex-shrink-0 pt-0.5 pb-0.5">
                                  {statusLabel}
                                </span>
                              </>
                            )}
