...
const StudentMonthlyView = () => {
  ...
  // Handle scroll to navigate to weekly dashboard - based on forcing scroll at top
  useEffect(() => {
    const handleScroll = (e) => {
      ...
      if (atTop && isScrollingUp) {
        ...
        // Reset if user scrolls away from top or scrolls down
        if (!atTop || !isScrollingUp) {
          forceScrollAttempts.current = 0;
          isAtTop.current = false;
        }
    ...
    const handleWheel = (e) => {
      ...
      if (distanceFromTop <= 5 && e.deltaY < 0) {
        ...
        // Reset if not at top or scrolling down
        forceScrollAttempts.current = 0;
      }
    };
