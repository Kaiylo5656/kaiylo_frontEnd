...
const AuthCallback = () => {
  ...
  useEffect(() => {
    ...
    logger.debug('🔍 URL search params:', Object.fromEntries(searchParams.entries()));
    ...
    logger.debug('🔍 OAuth callback params:', { urlError, errorDescription, hasCode: !!code });
    
    if (urlError) {
      logger.error('❌ OAuth error:', urlError, errorDescription);
    ...
    if (code) {
      logger.debug('✅ OAuth code found in URL, Supabase should process it automatically');
      logger.debug('⏳ Waiting for Supabase to process the code...');
      ...
      logger.debug('✅ URL hash found, Supabase should process it automatically');
    }
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange(
      async (event, session) => {
        logger.debug('🔄 Auth state changed:', event, session ? 'Session present' : 'No session');
        
        if ((event === 'SIGNED_IN' || event === 'INITIAL_SESSION') && session && session.access_token) {
          if (sessionProcessedRef.current) {
            logger.debug('⚠️ Session already processed, ignoring duplicate event');
            return;
          }
          sessionProcessedRef.current = true;
          
          logger.debug('✅ OAuth session established:', session.user.email);
